"""
Train a binary classifier on DNABERT-2 embeddings produced by ``dnarna.models.dna.dnabert2.embed``.

Example:
    python -m dnarna.models.dna.dnabert2.predict.train \
        --embeddings_npz data/dna_embeddings.npz \
        --metadata_file data/splits.parquet \
        --output_dir outputs/dnabert2_clf \
        --epochs 25 --hidden_dims 512,256 --device cuda:0

Required inputs:
    - embeddings_npz: Combined DNABERT-2 embeddings with ``embeddings`` + ``ids``.
...
LOGGER = logging.getLogger("dnabert2_train")
...
def _parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(
        description="Train a binary classifier to predict DNA labels from DNABERT-2 embeddings."
    )
    parser.add_argument(
        "--embeddings_npz",
        required=True,
