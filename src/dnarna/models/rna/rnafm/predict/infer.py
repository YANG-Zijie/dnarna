"""
Run inference with a trained RNA-FM classifier on precomputed embeddings.

Example:
    python -m dnarna.models.rna.rnafm.predict.infer \
        --embeddings_npz data/rna_embeddings.npz \
        --checkpoint outputs/rnafm_clf/model.pt \
        --output outputs/rnafm_clf/predictions.csv \
        --device cuda:0

Inputs:
    - embeddings_npz: .npz created by ``dnarna.models.rnafm.embed`` (expects
      ``embeddings`` + ``ids`` keys).
    - checkpoint: model.pt produced by ``dnarna.models.rna.rnafm.predict.train``.
...
LOGGER = logging.getLogger("rnafm_infer")
InferConfig = EmbeddingInferConfig


def _parse_args() -> InferConfig:
    parser = argparse.ArgumentParser(
        description="Run RNA-FM classifier inference on saved embeddings."
    )
    parser.add_argument(
        "--embeddings_npz",
        required=True,
